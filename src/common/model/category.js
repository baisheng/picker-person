...
export default class extends think.model.base {
     ...
     * @param conter_id
     */
    async getByTeamId(conter_id) {

        let list = await this.where({conter_id: conter_id, status: 1}).field('id,title as name,name as title, model, model_sub, pid,allow_publish,isapp,mold,description,display, cover').order('pid,sort').select();
    ...
    async get_colunm() {
        let lists = await this.where({status: 1}).field('id,title,name, model, model_sub, pid,allow_publish,isapp,mold,description,display, cover').order('pid,sort').select();
        for (let v of lists) {
            if (!think.isEmpty(v.title)) {
                v.url = `/${v.title}`
            } else {

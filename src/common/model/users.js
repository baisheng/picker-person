...
export default class extends Base {
    ...
    addUser(data, ip){
        ...
        let encryptPassword = this.getEncryptPassword(data.user_pass);

        return this.where({user_login: data.user_login, user_email: data.user_email, _logic: 'OR'}).thenAdd({
            user_login: data.user_login,
            user_email: data.email,
            display_name: data.display_name,
            user_pass: encryptPassword,
            user_registered: new Date().getTime(),
            user_activation_key: shortid.generate(),
    ...
    async active(data){
        ...
        // updateData.last_login_time = think.datetime();
        // updateData.last_login_ip = ip;
        return this.where({id: data.id}).update({user_status: 1});
    }
}